---
title: Andrew - 哈尔滨工业大学
not_in_nav: true
tags:
  - 工科
  - 跨省
  - 东北
  - 计算机科学
  - 985
---

# 我的飞跃故事

<div class="fy-case-hero" markdown="1">

## 基本信息

<div class="fy-case-info-grid" markdown="1">

<div class="fy-case-info-item">
  <div class="fy-case-info-label">化名</div>
  <div class="fy-case-info-value">Andrew</div>
</div>

<div class="fy-case-info-item">
  <div class="fy-case-info-label">毕业届数</div>
  <div class="fy-case-info-value">20 届</div>
</div>

<div class="fy-case-info-item">
  <div class="fy-case-info-label">高考分数</div>
  <div class="fy-case-info-value">545 分</div>
</div>

<div class="fy-case-info-item">
  <div class="fy-case-info-label">全省位次</div>
  <div class="fy-case-info-value">全省第 15797 名</div>
</div>

<div class="fy-case-info-item">
  <div class="fy-case-info-label">选科组合</div>
  <div class="fy-case-info-value">物理类（理科）</div>
</div>

<div class="fy-case-info-item">
  <div class="fy-case-info-label">录取院校</div>
  <div class="fy-case-info-value">哈尔滨工业大学</div>
</div>

<div class="fy-case-info-item">
  <div class="fy-case-info-label">录取专业</div>
  <div class="fy-case-info-value">计算机科学</div>
</div>

<div class="fy-case-info-item">
  <div class="fy-case-info-label">所在城市</div>
  <div class="fy-case-info-value">东北</div>
</div>

</div>

</div>

---

## 我为什么选择这个学校/专业

一轮内测：小范围的同学进行测试，并收集真实案例。进行完整的流程测试并进行优化。
二轮内测：进行批量宣发，主要针对熟悉老师的近几年之内的学生。扩充案例库开始形成社区文化。此时维护流程趋向于稳定。
三轮公测：与学校负责领导进行对接，进行官方宣发（公众号推文）。宣发内容主要是项目介绍、项目贡献邀请、维护团队招募、社区群建设（QQ群的建立和发展）。
四轮公测：进行近一个月的上线发布，在过程中维护和迭代优化。预期在26届学生出成绩开始填写志愿的节点，提供一个完整、完善的公共平台。

---

## 大学真实体验

### 学习压力

一轮内测：小范围的同学进行测试，并收集真实案例。进行完整的流程测试并进行优化。
二轮内测：进行批量宣发，主要针对熟悉老师的近几年之内的学生。扩充案例库开始形成社区文化。此时维护流程趋向于稳定。
三轮公测：与学校负责领导进行对接，进行官方宣发（公众号推文）。宣发内容主要是项目介绍、项目贡献邀请、维护团队招募、社区群建设（QQ群的建立和发展）。
四轮公测：进行近一个月的上线发布，在过程中维护和迭代优化。预期在26届学生出成绩开始填写志愿的节点，提供一个完整、完善的公共平台。

### 校园氛围

一轮内测：小范围的同学进行测试，并收集真实案例。进行完整的流程测试并进行优化。
二轮内测：进行批量宣发，主要针对熟悉老师的近几年之内的学生。扩充案例库开始形成社区文化。此时维护流程趋向于稳定。
三轮公测：与学校负责领导进行对接，进行官方宣发（公众号推文）。宣发内容主要是项目介绍、项目贡献邀请、维护团队招募、社区群建设（QQ群的建立和发展）。
四轮公测：进行近一个月的上线发布，在过程中维护和迭代优化。预期在26届学生出成绩开始填写志愿的节点，提供一个完整、完善的公共平台。

### 城市感受

一轮内测：小范围的同学进行测试，并收集真实案例。进行完整的流程测试并进行优化。
二轮内测：进行批量宣发，主要针对熟悉老师的近几年之内的学生。扩充案例库开始形成社区文化。此时维护流程趋向于稳定。
三轮公测：与学校负责领导进行对接，进行官方宣发（公众号推文）。宣发内容主要是项目介绍、项目贡献邀请、维护团队招募、社区群建设（QQ群的建立和发展）。
四轮公测：进行近一个月的上线发布，在过程中维护和迭代优化。预期在26届学生出成绩开始填写志愿的节点，提供一个完整、完善的公共平台。

### 宿舍与生活

一轮内测：小范围的同学进行测试，并收集真实案例。进行完整的流程测试并进行优化。
二轮内测：进行批量宣发，主要针对熟悉老师的近几年之内的学生。扩充案例库开始形成社区文化。此时维护流程趋向于稳定。
三轮公测：与学校负责领导进行对接，进行官方宣发（公众号推文）。宣发内容主要是项目介绍、项目贡献邀请、维护团队招募、社区群建设（QQ群的建立和发展）。
四轮公测：进行近一个月的上线发布，在过程中维护和迭代优化。预期在26届学生出成绩开始填写志愿的节点，提供一个完整、完善的公共平台。

### 资源与机会

一轮内测：小范围的同学进行测试，并收集真实案例。进行完整的流程测试并进行优化。
二轮内测：进行批量宣发，主要针对熟悉老师的近几年之内的学生。扩充案例库开始形成社区文化。此时维护流程趋向于稳定。
三轮公测：与学校负责领导进行对接，进行官方宣发（公众号推文）。宣发内容主要是项目介绍、项目贡献邀请、维护团队招募、社区群建设（QQ群的建立和发展）。
四轮公测：进行近一个月的上线发布，在过程中维护和迭代优化。预期在26届学生出成绩开始填写志愿的节点，提供一个完整、完善的公共平台。

### 社交与成长

一轮内测：小范围的同学进行测试，并收集真实案例。进行完整的流程测试并进行优化。
二轮内测：进行批量宣发，主要针对熟悉老师的近几年之内的学生。扩充案例库开始形成社区文化。此时维护流程趋向于稳定。
三轮公测：与学校负责领导进行对接，进行官方宣发（公众号推文）。宣发内容主要是项目介绍、项目贡献邀请、维护团队招募、社区群建设（QQ群的建立和发展）。
四轮公测：进行近一个月的上线发布，在过程中维护和迭代优化。预期在26届学生出成绩开始填写志愿的节点，提供一个完整、完善的公共平台。

---

## 应届生去向

一轮内测：小范围的同学进行测试，并收集真实案例。进行完整的流程测试并进行优化。
二轮内测：进行批量宣发，主要针对熟悉老师的近几年之内的学生。扩充案例库开始形成社区文化。此时维护流程趋向于稳定。
三轮公测：与学校负责领导进行对接，进行官方宣发（公众号推文）。宣发内容主要是项目介绍、项目贡献邀请、维护团队招募、社区群建设（QQ群的建立和发展）。
四轮公测：进行近一个月的上线发布，在过程中维护和迭代优化。预期在26届学生出成绩开始填写志愿的节点，提供一个完整、完善的公共平台。

---

## 我踩过的坑


<div class="fy-highlight-box" markdown="1">

#### ⚠️ 这是第一个坑

</div>

<div class="fy-highlight-box" markdown="1">

#### ⚠️ 这是第二个坑

</div>

<div class="fy-highlight-box" markdown="1">

#### ⚠️ 这是第三个坑

</div>

---

## 写给后来者的一句话

> "加油加油加油"
>
> —— Andrew

---

## 标签

<span class="fy-tag">工科</span>
<span class="fy-tag">跨省</span>
<span class="fy-tag">东北</span>
<span class="fy-tag">计算机科学</span>
<span class="fy-tag">985</span>
